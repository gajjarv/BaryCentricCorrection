...
![Example filterbank](https://github.com/gajjarv/BaryCentricCorrection/blob/master/pipeline_tests/Filterbank_example.png)
A typical filterbank file stores data in a time and frequency matrix format. Each row represents a sample, while each column represents a frequency channel for a given sample. Time is increasing from bottom to top while the frequency is increasing from right to left.
